...
const ROLE_OPTIONS = ['admin', 'manager', 'operator', 'viewer', 'GABINETE_VIEWER_GLOBAL', 'GOVERNO_GESTOR_GLOBAL'] as const
...
users.post('/', zValidator('json', userCreateSchema), async (c) => {
  ...
  try {
    await c.env.DB
      .prepare(
        `INSERT INTO users (id, name, email, password_hash, role, secretariat_id)
         VALUES (?, ?, ?, ?, ?, ?)`
      )
...
users.put('/:id', zValidator('json', userUpdateSchema), async (c) => {
  ...
  if (setClauses.length > 0) {
    ...
    const oldValues = {
      ...
      role: (existing as { role: string }).role,
      secretariat_id: (existing as { secretariat_id: string | null }).secretariat_id,
      is_active: (existing as { is_active: number }).is_active,
    }
...
users.put('/:id/roles', zValidator('json', userRolesSchema), async (c) => {
  ...
  await logAction(c.env.DB, 'user', userId, 'users.roles.update', {
    ...
    ip: c.req.header('CF-Connecting-IP') || undefined,
    userAgent: c.req.header('User-Agent') || undefined,
  }, { role: (existing as { role: string }).role }, { role: newRole })
